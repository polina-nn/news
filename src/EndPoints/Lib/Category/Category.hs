...
checkLogicPathForAddCateg ::
  ...
  -> m (Either ErrorTypes.AddEditCategoryError ( DataTypes.CreateCategoryRequest
                                               , [DataTypes.Category]))
      ...
      | CategoryHelpTypes.cur_path' req ==
          pathFromWords
            (take
               ...
               (wordsPath (CategoryHelpTypes.new_path' req))) = False
      | otherwise = True
...
curPathWithChildrenToZero eq mapCateg =
  case Map.lookup (CategoryHelpTypes.cur_path' eq) mapCateg -- change the current paf to zero
        of
    Just (idCat, _) ->
  ...
  where
    ...
    curPathChildrenToZeroChildren cur_path e mapCat =
      case Map.lookupGT cur_path mapCat -- find the children of the current path
            of
        ...
        Just (childPath, (childId, _)) ->
          if levelPath childPath > CategoryHelpTypes.cur_level' e -- it's definitely a baby
            then curPathChildrenToZeroChildren childPath e $
...
changeZeroWhithChildrenToNew req mapCat =
  if CategoryHelpTypes.new_path_stay_after_id' req == 0
    then changeZeroChildToNew "0" req $
  ...
  where
    ...
    changeZeroChildToNew path e mapCa =
      case Map.lookupGT path mapCa -- find the children of the current path knowing that they are
            of
        Nothing -> mapCa
        Just (childPath, (childId, _)) ->
          if levelPath childPath > 1
            then changeZeroChildToNew childPath e $
                 Map.delete childPath $
                 Map.insert
...
toFullReqiest ::
  ...
  -> [DataTypes.Category]
  -> m (Either ErrorTypes.AddEditCategoryError CategoryHelpTypes.EditCategoryFullRequest)
toFullReqiest h idCat DataTypes.EditCategoryRequest { DataTypes.new_path = mustbePath
...
toCategoriesEmptyName mapCateg = map toCategory $ Map.toList mapCateg
  where
    ...
    toCategory (path, (idCat, [])) =
      DataTypes.Category
        {category_id = idCat, category_path = path, category_name = ""}
...
validLogicPathForAddCateg categories DataTypes.CreateCategoryRequest {DataTypes.path = "1"} =
  ...
  Map.null (mapCategory categories) || False
validLogicPathForAddCateg categories DataTypes.CreateCategoryRequest {..}
  ...
  where
    catMap =
      mapCategory categories :: Map.Map DataTypes.Path ( DataTypes.Id
                                                       , DataTypes.Path)
