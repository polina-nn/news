...
addCategory _ (h, user, r) = do
  Logger.logInfo (News.hLogHandle h) $ T.pack "Request: Add Category "
    ...
    Right conn -> do
      ...
      case allCheck of
        ...
        Right (req, categories) -> do
          let toChangePaths = Category.changePathForAddCategory req categories
          rez <-
            catch
