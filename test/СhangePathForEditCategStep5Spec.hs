...
module СhangePathForEditCategStep5Spec
  ( spec,
  )
where
...
spec :: Spec
spec = describe "Category.changePathsForEditCateg. STEP5 Move Anna Borisovna to Anna's children" $ do
  it "Step1:" $
...
categoryAfterStepOne =
  [ DataTypes.Category {category_path = "0", category_id = 8, category_name = ""},
...
editCategoryRequests =
  Just
    [ CategoryHelpTypes.EditCategory {_id = 8, new_path = "1.1"},
      CategoryHelpTypes.EditCategory {_id = 9, new_path = "3.3.1"}
    ]
